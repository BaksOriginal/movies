...
let chatReadPollInterval = null; // Быстрый опрос статуса "Просмотрено" (для отзывчивого индикатора)
let chatReplyTarget = null; // Сообщение, на которое сейчас отвечаем (или null)
let otherUserLastRead = null; // Время последнего прочтения чата собеседником (ISO-строка)
let typingHideTimeout = null; // Таймер скрытия индикатора "печатает..."
let lastTypingSentAt = 0; // Троттлинг отправки broadcast-события печати
...
function subscribeToChanges() {
    ...
    realtimeChannel = db
        ...
        // Следим за отметками "прочитано" в чате (для индикатора "Просмотрено")
        .on(
            'postgres_changes',
        ...
        // Следим за широковещательными событиями "печатает..."
        .on(
            'broadcast',
            { event: 'typing' },
            (payload) => {
                const info = payload.payload || {};
                if (!currentUser || info.user_id === currentUser.id) return; // игнорируем собственную печать
...
async function onChatRealtimeChange() {
    ...
    if (isChatScreenOpen) {
        renderChatMessages();
        await markChatAsRead(); // новое сообщение увидено сразу же, раз экран чата открыт
    }
}

// =======================================================
// "ПРОСМОТРЕНО" — ОТМЕТКА О ПРОЧТЕНИИ ЧАТА СОБЕСЕДНИКОМ
// =======================================================
...
// Загружает время последнего прочтения чата собеседником
async function loadReadStatus() {
    const { data, error } = await db.from('chat_reads').select('user_id, last_read_at');
    if (error) {
        console.error("Ошибка при загрузке статуса прочтения:", error);
        return;
    }
    const otherRow = (data || []).find(r => !currentUser || r.user_id !== currentUser.id);
    otherUserLastRead = otherRow ? otherRow.last_read_at : null;
}

// Обновляет отметку "Просмотрено" под последним ИЗ МОИХ отправленных сообщений
function updateReadIndicator() {
    document.querySelectorAll(".chat-bubble-mine .chat-read-indicator").forEach(el => el.textContent = "");
...
// Отправляет широковещательное (broadcast) событие о том, что пользователь печатает
function broadcastTyping() {
    if (!currentUser || !realtimeChannel) return;
    const now = Date.now();
    if (now - lastTypingSentAt < 1500) return; // троттлинг, чтобы не спамить событиями
    lastTypingSentAt = now;

    realtimeChannel.send({
        type: 'broadcast',
        event: 'typing',
        payload: { user_id: currentUser.id, username: getUsernameFromEmail(currentUser.email) }
    });
}

// Показывает индикатор "печатает..." и автоматически прячет его через 3 секунды бездействия
...
function createChatBubble(msg) {
    ...
    // Индикатор "Просмотрено" под своим последним сообщением (заполняется отдельно)
    if (isMine) {
        let readEl = document.createElement("div");
        readEl.className = "chat-read-indicator";
...
async function showChatScreen() {
    ...
    app.appendChild(typingIndicator);
    ...
    await Promise.all([loadChatMessages(), loadReadStatus()]);
    renderChatMessages();
    updateReadIndicator();
    ...
    // Отдельный БЫСТРЫЙ опрос статуса прочтения — чтобы отметка "Просмотрено"
    // не запаздывала, даже если realtime-событие по таблице chat_reads
    // придёт с задержкой или не придёт вовсе
    if (chatReadPollInterval) clearInterval(chatReadPollInterval);
